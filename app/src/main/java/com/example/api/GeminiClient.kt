...
object GeminiClient {
    ...
    suspend fun generateReplies(
        ...
        if (apiKey.isBlank() || apiKey == "MY_GEMINI_API_KEY") {
            Log.w(TAG, "No valid Gemini API key configured, generating smart simulated preview replies.")
            return@withContext Result.success(generateFallbackReplies(question, count, length, tone))
        }
        ...
        try {
            ...
            if (!response.isSuccessful) {
                ...
                // Return fallback with error notification so user has replies
                val fallbacks = generateFallbackReplies(question, count, length, tone)
                return@withContext Result.success(fallbacks)
            }
            ...
            val parsedReplies = parseReplies(text, count, question, length, tone)
            Result.success(parsedReplies)
        } catch (e: Exception) {
            Log.e(TAG, "Exception calling Gemini API", e)
            Result.success(generateFallbackReplies(question, count, length, tone))
        }
    }
    ...
    private fun parseReplies(
        ...
        return if (filtered.isNotEmpty()) {
            ...
            generateFallbackReplies(question, targetCount, length, tone)
        }
    }
