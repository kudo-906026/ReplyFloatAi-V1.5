...
- **📊 Real-Time Diagnostics & Telemetry**:
  ...
  - Deep telemetry for screen capture frames, bitmap conversion, and raw ML Kit OCR output.
...
```
         ...
         Fast Text Extraction (~3ms)              ▼
                    ...
                    │                             │
                    │                             ▼
                    │                    Frame Buffer Acquired
                    │                             │
                    │                             ▼
                    │                   On-Device ML Kit OCR
                    │                  Text Recognition (~50ms)
                    │                             │
                    └───────────────┬─────────────┘
                                    │
                                    ▼
...
- **Diagnostics Tab**:
  ...
  - Inspect on-device screenshot capture status, frame telemetry, system error codes, and raw ML Kit OCR text output.
  - Filter, inspect, and export or clear logs at any time.

---

## 🔑 Persistent Signing & Update Installation (GitHub CI)

To allow APKs generated by GitHub Actions to install directly as updates without requiring an uninstall:
- **Consistent Application ID**: `com.aistudio.replyfloat.kqpzvx` is fixed and permanent across all builds.
- **Automated Keystore Restoration**: GitHub Actions automatically restores the persistent signing keystore into both the workspace root and `~/.android/debug.keystore` before Gradle executes.
- **Repository Secret Support (Optional)**: If you want to use a custom release keystore, add a repository secret named `RELEASE_KEYSTORE_BASE64` under **Settings > Secrets and variables > Actions** containing your base64-encoded keystore. If omitted, the workflow automatically uses the repository's committed persistent signing key so signatures remain 100% stable across every commit and run.
